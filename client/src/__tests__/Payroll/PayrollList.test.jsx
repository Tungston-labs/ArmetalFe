...
import PayrollTable from "./PayrollTable"; // adjust path/filename to match your project
...
jest.mock("../../Components/VerificationCircle", () => (props) => (
...
jest.mock("../../Components/payroll/IncentiveModal/IncentiveModal", () => (props) => (
  <div data-testid="incentive-modal">
    <button onClick={() => props.onClose(true)}>Save Incentive</button>
  </div>
));
jest.mock("../../Components/payroll/DeductionModal/DeductionModal", () => (props) => (
...
describe("PayrollTable", () => {
  ...
  test("verification click blocked if the same HR already verified this employee", () => {
    ...
    expect(Swal.fire).toHaveBeenCalledWith(
      expect.objectContaining({ icon: "warning", title: "Cannot Verify" })
    );
    expect(payrollSlice.verifyEmployeePayroll).not.toHaveBeenCalled();
  });
